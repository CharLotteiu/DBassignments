							...
							<a href="#" class="dropdown-toggle icon-menu" data-toggle="dropdown">
								...
								<?php
									...
									while($row = mysqli_fetch_array($ddl_result,MYSQLI_ASSOC)){
                                        ...
                                        else if($row['ddl']==date('Y-m-d',strtotime("-2 day"))){
			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li>
							<a href="#subPages" data-toggle="collapse" class="collapsed active"><i class="lnr lnr-menu"></i> <span>清单列表</span> <i class="icon-submenu lnr lnr-chevron-left"></i></a>
                            ...
                            <div class="panel" id="todolist">
                                <div class="panel-heading">
									...
									<div class="right">
                                        ...
                                        <button title="筛选To-Do" type="button" class="btn-lg"><a href="index.php"><i class="lnr lnr-magnifier"></i></a></button>
                                    </div>
                                </div>
                                <div class="panel-body">
									...
									<?php
											...
											if(isset($_POST['userid'])){ 
												...
												//$list = $_POST['list']; //仅能查看本清单内容，故不再设置
												...
												$select = "SELECT tid, content, ddl, importance, status 
															 FROM todo";
												//根据获取到的筛选条件数量拼合SQL语句
												$todo_sql = $select.$list_sql.'
															WHERE '.implode(" AND ", $where).' 
															AND lid="'.$_GET['lid'].'"
															ORDER BY status asc, ddl asc';	
												//echo $todo_sql;	
											...
											//Edited by 刘玲玲
												...
												$todo_sql = 'SELECT tid, content, ddl, importance, status
															 FROM todo 
															 WHERE uid ="'.$_SESSION['userid'].'" AND lid="'.$_GET['lid'].'" ORDER BY status asc, ddl asc';
											}											
											
											$ii = 0;
											$todo_result = mysqli_query($conn,$todo_sql) or die(mysqli_error($conn));
                                            if(mysqli_num_rows($todo_result)==0){
                                                echo "非常抱歉，好像这个清单下面暂时没有内容呢......";
                                            }
                                            else{
                                                while($row = mysqli_fetch_array($todo_result,MYSQLI_ASSOC)){
                                                    ...
                                                    if($row['status']==1){
                                                        ...
                                                        <input type="checkbox" id="'.$ii.'_sel" onClick="status_toggle('.$ii.','.$row['tid'].','.$row['status'].','.$_GET['lid'].',\''.$_GET['name'].'\')" checked="checked">'.'<span></span>
                                                        </label>
                                                        <p>
                                                            <a href="javascript:void(0);" style="color:gray" onClick="EditTodo('.$row['tid'].');"><span class="title" style="margin-right:5px">'.$row['content']."   </span><span class='label label-success'>已完成</span>
                                                        ...
                                                        <input type="checkbox" id="'.$ii.'_sel" onClick="status_toggle('.$ii.','.$row['tid'].','.$row['status'].','.$_GET['lid'].',\''.$_GET['name'].'\')">'.'<span></span>
                                                        </label>
                                                        <p>
                                                            <a href="javascript:void(0);" onClick="EditTodo('.$row['tid'].');"><span class="title" style="margin-right:5px">'.$row['content']." </span></a><span class='label label-warning'>进行中</span>
                                                            <span class='short-description'><span class='star_rate'></span><input class='impo' type='hidden' value='".$row['importance']."'></input>";
                                                    }
                                                    else{
                                                        echo '<li id="'.$ii.'"><label class="control-inline fancy-checkbox">
                                                        ...
                                                        <p>
                                                            <a href="javascript:void(0);" onClick="EditTodo('.$row['tid'].');"><span class="title" style="margin-right:5px">'.$row['content']." </span></a><span class='label label-danger'>已过期</span>
                                                            <span class='short-description'><span class='star_rate'></span><input class='impo' type='hidden' value='".$row['importance']."'></input>";
                                                    }
                                                    $ii++;
	...
	function status_toggle(ele_id,tid, status){
        $.ajax({
            ...
            success:function(result){
				...
				$( "#todo" ).load( "listdetail.php #todolist", function( response, status, xhr ) {
					if ( status == "error" ) {
						var msg = "对不起，但是有一个错误: ";
						$( "#error" ).html( msg + xhr.status + " " + xhr.statusText );
					}
